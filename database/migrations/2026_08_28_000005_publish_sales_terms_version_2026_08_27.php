...
return new class extends Migration
{
    ...
    {
        $sourcePath = resource_path('views/pages/legal/sales.blade.php');
        $source = file_get_contents($sourcePath);

        if (! is_string($source)
            || ! preg_match("/@section\('legal-content'\)\s*(.*?)\s*@endsection/s", $source, $matches)) {
            throw new RuntimeException('Unable to load the sales terms dated 2026-08-27.');
        }
        ...
        DB::transaction(function () use ($content, $hash, $publishedAt) {
            $version = DB::table('legal_document_versions')
                ->where('document_key', 'sales')
                ->where('locale', 'it')
                ->where('version', '2026-08-27')
                ->first();

            if ($version && ! hash_equals((string) $version->content_hash, $hash)) {
                throw new RuntimeException('A different sales terms version 2026-08-27 already exists.');
            }
            ...
            $versionId = $version?->id ?? DatabaseUuid::insert('legal_document_versions', [
                ...
                'content_format' => 'html',
                'source_path' => 'resources/views/pages/legal/sales.blade.php',
