...
return new class extends Migration
{
    ...
    {
        $sourcePath = resource_path('views/pages/legal/terms.blade.php');
        $source = file_get_contents($sourcePath);

        if (! is_string($source)
            || ! preg_match("/@section\('legal-content'\)\s*(.*?)\s*@endsection/s", $source, $matches)) {
            throw new RuntimeException('Unable to load the waitlist terms version 1.2.');
        }
        ...
        DB::transaction(function () use ($content, $hash, $publishedAt) {
            ...
            if ($version && ! hash_equals((string) $version->content_hash, $hash)) {
                throw new RuntimeException('A different waitlist terms version 1.2 already exists.');
            }
            ...
            $versionId = $version?->id ?? DatabaseUuid::insert('legal_document_versions', [
                ...
                'content_format' => 'html',
                'source_path' => 'resources/views/pages/legal/terms.blade.php',
