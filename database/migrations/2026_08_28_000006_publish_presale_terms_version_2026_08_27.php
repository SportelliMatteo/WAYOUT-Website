...
return new class extends Migration
{
    ...
    {
        $sourcePath = resource_path('views/pages/legal/presale.blade.php');
        $source = file_get_contents($sourcePath);

        if (! is_string($source)
            || ! preg_match("/@section\('legal-content'\)\s*(.*?)\s*@endsection/s", $source, $matches)) {
            throw new RuntimeException('Unable to load the pre-sale terms dated 2026-08-27.');
        }
        ...
        DB::transaction(function () use ($content, $hash, $publishedAt) {
            ...
            $versionId = $version?->id ?? DatabaseUuid::insert('legal_document_versions', [
                ...
                'content_format' => 'html',
                'source_path' => 'resources/views/pages/legal/presale.blade.php',
            ...
            DB::table('legal_documents')->updateOrInsert(
                ['document_key' => 'presale', 'locale' => 'it'],
                [
