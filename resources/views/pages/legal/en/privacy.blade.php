...
<div class="space-y-8">
    ...
    <section>
        ...
        <p class="mt-3">In relation to identity and checkout, WAYOUT uses Google’s Firebase Phone Authentication and reCAPTCHA to check the phone number, prevent abuse and get the technical token needed to create or recognize the WAYOUT backend account. The Firebase token is treated temporarily from the site and forwarded to the backend; the verified number is stored in the account profile. In relation to payments, WAYOUT receives from the WAYOUT backend and Stripe the information necessary to reconcile the order, including outcome, amount, currency and session/transaction identifiers. Full card data and CVC are not acquired or stored directly by WAYOUT.</p>
    ...
    <section>
        ...
        <ul class="mt-3 list-disc space-y-2 pl-6">
            ...
            <li>Google, for Google Tag Manager and Google Analytics 4, and Meta Platforms, for Meta Pixel browser side, only after relevant consent and according to their privacy roles;</li>
    ...
    <section>
        ...
        <p class="mt-3">The site uses cookies and technical tools necessary for the operation of the Laravel application, session management, CSRF protection, storage of language preference and registration of cookies choices. In checkout, Firebase Phone Authentication and reCAPTCHA can use technical identifiers, network requests and storage that are strictly necessary for the verification of phone and prevention of abuse. Stripe payment tools are used in checkout managed by the WAYOUT backend. The tools strictly necessary are used only for the required function and not for advertising purposes, without prejudice to the obligation of information.</p>
        ...
        <p class="mt-3">After Marketing consent, WAYOUT uses Meta Pixel exclusively browser side to measure campaign visits and conversions, understand the effectiveness of advertising communication and, where enabled, create customized publics or perform remarketing. Meta can receive online identifiers, cookies, IP address, browser and device information, page visited, referrer and events on the site. At the date of this version, Advanced Matching and Conversions API are disabled.</p>
