...
<div style="max-width:600px;margin:0 auto;padding:32px 20px">
    ...
    <div style="background:#fff;border-radius:20px;padding:28px;margin-top:16px">
        ...
        @if (! empty($purchase['order_reference']))
            <p>{{ __('messages.email.order_number') }}: <strong>{{ $purchase['order_reference'] }}</strong></p>
        @endif
        ...
        @elseif (! empty($purchase['invoice_requested']))
            ...
            @endif
        @endif
        <p><a href="{{ route('legal.sales', ['lang' => app()->getLocale()]) }}" target="_blank" style="color:#6d28d9;font-weight:bold">{{ __('messages.email.sales_terms_link') }}</a> · <a href="{{ route('legal.presale', ['lang' => app()->getLocale()]) }}" target="_blank" style="color:#6d28d9;font-weight:bold">{{ __('messages.legal.presale') }}</a> · <a href="{{ route('legal.refunds', ['lang' => app()->getLocale()]) }}" target="_blank" style="color:#6d28d9;font-weight:bold">{{ __('messages.email.refunds_link') }}</a> · <a href="{{ route('legal.refunds', ['lang' => app()->getLocale()]) }}#recedere" target="_blank" style="color:#6d28d9;font-weight:bold">{{ __('messages.withdrawal.withdraw_here') }}</a></p>
